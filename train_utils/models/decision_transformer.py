...
class DecisionTransformer2(TrajectoryModel):
    ...
    def __init__(
        ...
        self.embed_state    = nn.ModuleList([torch.nn.Linear(self.state_dim, hidden_size) for _ in range(state_length)])
    ...
    def forward(self, states, actions, poses, command,returns_to_go, timesteps, attention_mask=None):
        ...
        state_embeddings = [self.embed_state[i](states[:,:,i]) for i in range(self.state_length)] 
        ...
        input_sequence = [returns_embeddings,pos_embeddings,command_embeddings]+ state_embeddings +[action_embeddings]
        
        stacked_inputs = torch.stack(input_sequence, dim=1)
        stacked_inputs = stacked_inputs.permute(0, 2, 1, 3)
        stacked_inputs = stacked_inputs.reshape(batch_size, self.step_length*seq_length, self.hidden_size)
        stacked_inputs = self.embed_ln(stacked_inputs)

        # to make the attention mask fit the stacked inputs, have to stack it as well
        stacked_attention_mask = torch.stack(
            [attention_mask]*self.step_length, dim=1
        ).permute(0, 2, 1).reshape(batch_size, self.step_length*seq_length)
        ...
        # returns (0) pos (1), command (2), state (3:8) , action (8); i.e. x[:,1,t] is the token for s_t
        x = x.reshape(batch_size, seq_length, self.step_length, self.hidden_size).permute(0, 2, 1, 3)
...
class DL_model(arch):
    def __init__(self,args) -> None:
        ...
        self.dl_model = DecisionTransformer2(
            state_dim=args.imgs_emps//5,
        ...
        if args.model_eval:
            self.states_embeddings   = deque([torch.zeros(1, 5,args.imgs_emps//5).to(self.dummy_param.device) for _ in range(args.seq_len)], maxlen=args.seq_len)  
    ...
    def eval_step(self,input_step):
        ...
        action_preds = self.dl_model.forward(
        states_embeddings, actions,poses_embeddings,commands_embeddings,returns_to_go.unsqueeze(-1), timesteps, attention_mask=attention_mask)
