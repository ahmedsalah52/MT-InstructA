...
class MLP(nn.Module):
    ...
    def forward(self, x,lora=None):
        if lora: 
            x = self.c_fc(x) + lora.ff1_ab(x)
        else:
            x = self.c_fc(x)
        x = self.gelu(x)
        if lora:
            x = self.c_proj(x) + lora.ff2_ab(x)
        else:
            x = self.c_proj(x)
        x = self.dropout(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x,lora=None):
        x = x + self.attn(self.ln_1(x),lora)
        x = x + self.mlp(self.ln_2(x),lora)
        return x
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        #not easily readable, bool_size list of gpt list of modules and action module, gpt has list of layers, each layer has q_ab, v_ab, ff1_ab, ff2_ab
        self.lora = nn.ModuleList([
            nn.ModuleDict(dict(
            gpt = nn.ModuleList([nn.ModuleDict(dict(
                q_ab =   LoRA_layer(in_embed_dim= config.n_embd      ,out_embed_dim= config.n_embd      ,rank=config.lora_rank ,lora_alpha=config.lora_alpha),
                v_ab =   LoRA_layer(in_embed_dim= config.n_embd      ,out_embed_dim= config.n_embd      ,rank=config.lora_rank ,lora_alpha=config.lora_alpha),
                ff1_ab = LoRA_layer(in_embed_dim= config.n_embd      ,out_embed_dim= 4 * config.n_embd  ,rank=config.lora_rank ,lora_alpha=config.lora_alpha),
                ff2_ab = LoRA_layer(in_embed_dim= 4 * config.n_embd  ,out_embed_dim= config.n_embd      ,rank=config.lora_rank ,lora_alpha=config.lora_alpha),
                )) for _ in range(config.n_layer)]),
            action=  LoRA_layer(in_embed_dim= config.n_embd      ,out_embed_dim= config.action_size ,rank=config.lora_rank ,lora_alpha=config.lora_alpha)
    ...
    def forward(self, commands,returns_to_go, states, hand_poses, actions,time_steps,idx):
        b, t , _ = states.size()
        if not self.use_task_idx:
            idx = self.pool(commands.squeeze(1))
        ...
        actions_pred = self.action_head(stacked_sequence[:,-2]) + self.lora[idx].action(stacked_sequence[:,-2]) # step  -> return, state , hand_pos, actions # we predict the actions after the hand_pos 
...
class DT_lora(arch):
    ...
    def forward(self,batch):
        ...
        action_preds,rewards_preds = self.dt_model(
            ...
            idx = batch['task_id'][0].squeeze(-1) if self.use_task_idx else None
