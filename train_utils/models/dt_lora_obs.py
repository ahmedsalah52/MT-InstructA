...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x,lora_params=None):
...
class MLP(nn.Module):
    ...
    def forward(self, x,lora_params=None):
        if lora_params: 
            x = self.c_fc(x) + torch.matmul(x,lora_params['ff1_ab'])
        else:
            x = self.c_fc(x)
        x = self.gelu(x)
        if lora_params:
            x = self.c_proj(x) + torch.matmul(x,lora_params['ff2_ab'])
        else:
            x = self.c_proj(x)
        x = self.dropout(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x,lora_params=None):
        x = x + self.attn(self.ln_1(x),lora_params)
        x = x + self.mlp(self.ln_2(x),lora_params)
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, commands,returns_to_go, states, hand_poses, actions,time_steps,idx):
        ...
        lora_layers = self.lora.extract_prompt(idx)
...
class DL_lora_obs(arch):
    ...
    def eval_step(self,input_step):
        ...
        action_preds,rewards_preds,task_pred = self.dl_model(
            ...
            actions=actions,
            time_steps=timesteps,
            idx = input_step['task_id'] if self.use_task_idx else None
        )
