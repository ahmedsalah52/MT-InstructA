...
while(1):
    ...
    solutions = glob.glob(os.path.join('logs/episodes/trues',file_name+'*'))
    print(solutions)
    if len(solutions)>0: 
...
for i in range(400):
    rec_obs = solution_dict['obs'][i]

    #a = policy.get_action(obs)
    while(1):
        a = update(obs[0:3] ,  rec_obs[0:3] ) #rec_obs[0:3] - obs[0:3]                               # move(obs[0:3], to_xyz=rec_obs[0:3], p=40.)
        obs, reward, done, info = env.step(a*20)  # Step the environoment with the sampled random action
        if  np.sqrt(((obs[0:3] - rec_obs[0:3]) ** 2).mean())<0.001: break
        print(np.sqrt(((obs[0:3] - rec_obs[0:3]) ** 2).mean()))
    
    print(i,'action ',a, 'rec reward',solution_dict['reward'][i] ,' reward ' ,round(reward,2),' state ',info['success'])
    x = y = z = g = 0
    #env.render() 
